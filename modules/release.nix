...
let
  ...
  wrapScript = { commands, keysDir }: ''
    export PATH=${lib.makeBinPath releaseScriptDeps}:$PATH
...
in
{
  ...
  config.build = rec {
    ...
    releaseScript =
      (lib.warnIf (!config.signing.enable) "releaseScript should be used only if signing.enable = true; Otherwise, the build might be using incorrect keys / certificate metadata")
    ...
    # This contains all the information that is required to generate a signed release. If you sign on the build
    # host, using releaseScript or factoryImg is more convenient. Copy it with nix-copy-closure; or use some tool that
    # follows symlinks (e.g. scp or `cp -L`) if you don't want to put the release into Nix store on the signing
    # host. 
    releaseScriptInfo = pkgs.runCommand "robotnix-${config.productName}-${config.buildNumber}-release-info"
    { inherit unsignedTargetFiles otaMetadata;
      inherit (config.build) generateKeysInfo;
      passAsFile = [ "json" ];
      json = builtins.toJSON {
        inherit (config) productName buildNumber device channel otaArgs;
        updaterFlavor = config.apps.updater.flavor;
        unsignedTargetFiles = builtins.baseNameOf unsignedTargetFiles;
        signing = {
          inherit (config.signing) enable signTargetFilesArgs;
          #NOTE This means that this part of the build depends on keys for other devices.
          #TODO Filter by what is requested in generateKeysInfo. This is currently unused but we may want to check the fingerprints later.
          #inherit (config.signing) keyStoreMetadata;
          fingerprints = let
            nullOnError = value:
              let x = builtins.tryEval value; in
              if x.success then x.value else null;
  
            imageKeys = {
              "${config.device}/avb_pkmd.bin" = nullOnError config.signing.avb.fingerprint;
              "${config.device}/verity.x509.pem" = nullOnError config.signing.avb.verityFingerprint;
            };

            enabledPrebuiltApps = lib.filter (p: p.enable && p.apk != null) (lib.attrValues config.apps.prebuilt);
            signedPrebuiltApps = lib.filter (p: p.certificate != "PRESIGNED") enabledPrebuiltApps;
            appKeys = builtins.listToAttrs (map (p: lib.attrsets.nameValuePair "${p.certificate}.x509.pem" p.fingerprint) signedPrebuiltApps);
          in lib.attrsets.filterAttrs (k: v: v != null) (imageKeys // appKeys);
        };
      };
      #FIXME factory image is generated by: ${pkgs.runtimeShell} ${config.source.dirs."device/common".src}/generate-factory-images-common.sh
    }
    ''
      mkdir $out
      mv $jsonPath $out/release.json
      ln -s $unsignedTargetFiles $out/
      ln -s $generateKeysInfo $out/keys.json
      ln -s $otaMetadata $out/ota-metadata.txt
    '';

    releaseScriptShell = pkgs.mkShell "robotnix-release" {
      packages = releaseScriptDeps;
    };
  };
}
